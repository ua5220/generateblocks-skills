...
1. **Wrong block type for content** - Raw HTML (SVGs, text) inside `generateblocks/element` blocks. Element blocks expect inner blocks, not raw HTML content. Use `generateblocks/shape` for SVGs and `generateblocks/text` for text content
2. **Hover/transition CSS in `css` attribute** - The `css` attribute should contain only base styles. Hover states and transitions are generated by the plugin from the `styles` object
3. **htmlAttributes as array** - Must be a plain object: `{"href":"https://example.com/"}` not `[{"attribute":"href","value":"/url/"}]`
4. **className missing uniqueId** - Must be `"gb-element-{id} gb-element"` not just `"gb-element"`
5. **Relative URLs in htmlAttributes** - Use full absolute URLs (e.g., `https://example.com/contact/` not `/contact/`)
6. **Spaces in CSS functions** like `clamp()` - The block editor minifies CSS, so `clamp(2rem, 5vw, 3rem)` becomes `clamp(2rem,5vw,3rem)` causing a mismatch. Write without spaces from the start.
7. **Wrong SVG attribute order** in HTML - Must be `viewBox` first, then `fill`, then `aria-hidden`
8. **Non-compact nesting** - Closing tags must be on the same line as the parent closing comment, with no blank lines between them
